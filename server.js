...
wss.on('connection',(ws)=>{
  ...
  ws.on('message',(data)=>{
    ...
    if(msg.t==='create'){
      // Idempotent create: clients may retry when a proxy briefly delays a message.
      // If this socket already owns a room, simply resend the room information.
      if(attached&&player){
        send(ws,{t:'joined',id:player.id,slot:player.slot,code:attached.code,host:attached.host===player.id,started:attached.started,max:MAX_PLAYERS});
        send(ws,{t:'room',role:attached.host===player.id?'host':'client',code:attached.code,max:MAX_PLAYERS,started:attached.started});
        send(ws,{t:'lobby',count:attached.players.size,max:MAX_PLAYERS,started:attached.started,code:attached.code});
        return;
      }
      const code=roomCode();
      const room=createRoom(code,!!msg.solo);
      player=newPlayer(0,msg.name,msg.color);
      joinRoom(room,player,ws);
      attached=room;
      room.started=!!msg.solo;
      seedOrbs(room,28);
      send(ws,{t:'room',role:'host',code,max:MAX_PLAYERS,started:room.started});
      send(ws,{t:'lobby',count:room.players.size,max:MAX_PLAYERS,started:room.started,code});
      return;
    }
    if(msg.t==='join'){
